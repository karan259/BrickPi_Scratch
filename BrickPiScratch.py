...
class SetupNotRunError(Exception):
    pass
...
class myThread (threading.Thread):      #This thread is used for continuous transmission to BPi while main thread takes care of Rx/Tx Broadcasts of Scratch
    ...
    def run(self):
        global updateSensorValueRunning
        print "starting thread"
        while running:
            if updateSensorValueRunning==0:
                updateSensorValueRunning=1
                BrickPiUpdateValues()       # Ask BrickPi to update values for
    ...
    try:
            ...
            for i in range(4):
                if sensor[i]:
                    if spec[i]:
                        ...
                        if debug:
                            print "comp:",sensorbroadcasts[i],BrickPi.Sensor[PORT_1+i],"\n"
                    else:                
		...
		while True:
			...
			try:
				...
				print "BrickPi Scratch: Scratch is either not opened or remote sensor connections aren't enabled\n..............................\n"
    except SetupNotRunError as e:
        print "\nSETUP message was not broadcast from Scratch. Need SETUP broadcast message to start"
